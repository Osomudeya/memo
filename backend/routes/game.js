...
const { validateGameStart, validateCardMatch, validateGameComplete } = require('../middleware/validation');
...
const { sanitizeInput, validateJsonContent } = require('../middleware/validation');
...
router.post('/start', validateGameStart, async (req, res) => {
    try {
        ...
        console.log(`🎮 New ${difficulty} game started by ${username} (Game ID: ${game.id})`);
...
router.post('/match', validateCardMatch, async (req, res) => {
    try {
        ...
        console.log(`${isMatch ? '✅' : '❌'} Match attempt by ${gameSession.username}: ${card1.emoji} + ${card2.emoji} = ${isMatch ? 'SUCCESS' : 'MISS'}`);
...
router.post('/complete', validateGameComplete, async (req, res) => {
    try {
        ...
        console.log(`🏆 Game completed by ${gameSession.username}! Score: ${scoreBreakdown.totalScore}, Time: ${timeElapsed}ms`);
...
router.get('/:gameId', async (req, res) => {
    try {
        ...
        res.json({
            success: true,
            game: gameSession,
            config: gameConfig.difficulties[gameSession.difficulty] || gameConfig.difficulties.easy
        });
...
function calculateConsecutiveMatches(matches) {
    ...
    for (let i = 1; i < sortedMatches.length; i++) {
        const timeDiff = sortedMatches[i].timestamp - sortedMatches[i-1].timestamp;
