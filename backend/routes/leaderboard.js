...
function getPlayerBadge(score, rank) {
    ...
    if (score >= 300) return { emoji: '🧠', title: 'Memory Master', color: '#9932CC' };
    ...
    if (score >= 100) return { emoji: '🌱', title: 'Rising Star', color: '#90EE90' };
...
function isRecentlyActive(lastPlayed) {
    ...
    const daysSinceLastPlayed = (Date.now() - new Date(lastPlayed)) / (1000 * 60 * 60 * 24);
...
function generateInsights(overallStats, difficultyStats, records) {
    ...
    insights.push(`🎮 Most popular difficulty: ${popularDifficulty.difficulty} (${popularDifficulty.gamesPlayed} games)`);
    if (records.highest_score && records.highest_score.value >= 300) {
        insights.push(`🏆 Incredible high score of ${records.highest_score.value} points by ${records.highest_score.username}!`);
    }
    if (records.fastest_time && records.fastest_time.value < 60000) {
        insights.push(`⚡ Lightning fast completion in ${records.fastest_time.formatted} by ${records.fastest_time.username}!`);
    }
    if (overallStats.activeUsers24h >= 5) {
        insights.push(`🔥 Active community with ${overallStats.activeUsers24h} players in the last 24 hours!`);
    }
    return insights;
}
...
router.get('/', async (req, res) => {
    try {
        ...
        console.log(`🏆 Fetching leaderboard (limit: ${limit}, timeframe: ${timeframe})`);
        let filteredLeaderboard = [...mockLeaderboard];
        if (timeframe === 'week') {
            const oneWeekAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
            filteredLeaderboard = filteredLeaderboard.filter(player => 
                new Date(player.last_played) > oneWeekAgo
            );
        } else if (timeframe === 'month') {
            const oneMonthAgo = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
            filteredLeaderboard = filteredLeaderboard.filter(player => 
                new Date(player.last_played) > oneMonthAgo
            );
        }
        ...
        const formattedLeaderboard = filteredLeaderboard.map((player, index) => ({
            ...
            timeFormatted: player.best_time ? `${(player.best_time / 1000).toFixed(1)}s` : 'N/A',
        ...
        res.json({
            ...
            metadata: {
                ...
                topScore: formattedLeaderboard.length > 0 ? formattedLeaderboard[0].bestScore : 0,
                averageScore: formattedLeaderboard.length > 0 
                    ? Math.round(formattedLeaderboard.reduce((sum, p) => sum + p.bestScore, 0) / formattedLeaderboard.length)
                    : 0
            },
...
router.get('/fresh', async (req, res) => {
    try {
        ...
        const formattedLeaderboard = mockLeaderboard.slice(0, parseInt(limit)).map((player, index) => ({
            rank: index + 1,
            username: player.username,
            displayName: player.display_name || player.username,
            totalGames: player.total_games,
...
router.get('/rank/:username', async (req, res) => {
    try {
        ...
        const userIndex = mockLeaderboard.findIndex(player => player.username === username);
        if (userIndex === -1) {
        ...
        const endIndex = Math.min(mockLeaderboard.length - 1, userIndex + contextSize);
        const contextPlayers = mockLeaderboard.slice(startIndex, endIndex + 1).map((player, idx) => ({
            ...
            pointsToNext: startIndex + idx > 0 ? 
                mockLeaderboard[startIndex + idx - 1].best_score - player.best_score : 0
        }));
        res.json({
            success: true,
            userRank: {
            ...
            statistics: {
                ...
                percentile: ((mockLeaderboard.length - userRank + 1) / mockLeaderboard.length * 100).toFixed(1),
...
router.get('/stats', async (req, res) => {
    try {
        ...
        const totalGames = mockLeaderboard.reduce((sum, player) => sum + player.total_games, 0);
        const highestScore = Math.max(...mockLeaderboard.map(p => p.best_score));
        const averageScore = mockLeaderboard.reduce((sum, p) => sum + p.best_score, 0) / totalPlayers;
        const fastestTime = Math.min(...mockLeaderboard.map(p => p.best_time));
        const activeUsers24h = mockLeaderboard.filter(player => 
        ...
        const statistics = {
            ...
            records: {
                ...
                fastest_time: {
                    username: mockLeaderboard.find(p => p.best_time === fastestTime)?.username,
                    value: fastestTime,
                    unit: 'ms',
        ...
        const insights = generateInsights(statistics.overall, statistics.byDifficulty, statistics.records);
        res.json({
            success: true,
            statistics,
            insights,
