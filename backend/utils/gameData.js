...
const emojiPairs = [
    ...
    { id: 'thinking', emoji: '🤔', name: 'Thinking Face', category: 'thoughtful' },
    ...
    { id: 'trophy', emoji: '🏆', name: 'Victory Trophy', category: 'achievement' },
...
class GameDataGenerator {
    ...
    static getRandomEmojiByCategory(category) {
        const categoryEmojis = emojiPairs.filter(emoji => emoji.category === category);
    ...
    static calculateScore(gameStats) {
        ...
        if (timeElapsed < config.timeLimit * 0.5) { // Finished in less than half the time limit
            breakdown.speedBonus = Math.floor(breakdown.baseScore * scoring.speedBonusMultiplier);
    ...
    static getPerformanceRating(score, difficulty) {
        const config = gameConfig.difficulties[difficulty];
        const maxPossibleScore = (config.cardCount / 2) * config.pointsPerMatch + 
                                 config.bonusPoints + 
    ...
    static getDailySpecialRule(seed) {
        const rules = [
            { name: 'Speed Demon', description: 'Double points for matches under 5 seconds', type: 'speed' },
            { name: 'Category Master', description: 'Bonus points for matching specific categories in order', type: 'category' },
            { name: 'Perfect Memory', description: 'No wrong moves allowed!', type: 'perfect' },
            { name: 'Chain Reaction', description: 'Consecutive matches give increasing bonuses', type: 'streak' },
            { name: 'Time Crunch', description: 'Half the usual time limit', type: 'time' }
        ];
