...
async function apiRequest(endpoint, options = {}) {
    try {
        ...
        console.log(`✅ API Response: ${endpoint}`, data.success ? 'Success' : 'Info');
        return data;
    } catch (error) {
...
async function startNewGame() {
    try {
        ...
        if (!/^[a-zA-Z0-9]+$/.test(username)) {
            showNotification('Username can only contain letters and numbers! 🔤', 'error');
            return;
        }
...
async function loadLeaderboard(fresh = false) {
    try {
        ...
        // Show loading state
        leaderboardContent.innerHTML = '<div class="loading-spinner">📊 Loading leaderboard...</div>';
...
async function loadUserStats() {
    ...
    try {
        const result = await apiRequest(`/scores/${gameState.currentUser.username}?includeHistory=true`);
...
function setupEventListeners() {
    ...
    if (refreshLeaderboard) refreshLeaderboard.addEventListener('click', () => loadLeaderboard(true));
...
function setupGameBoard() {
    ...
    console.log(`🃏 Created ${gameState.cards.length} cards for ${gameState.difficulty} difficulty`);
}
...
function handleMatchSuccess(card1, card2, result) {
    ...
    console.log(`✅ Match found: ${result.match?.card1.emoji} + ${result.match?.card2.emoji}`);
}
...
function showGameCompleteModal(gameResult, achievements) {
    ...
    document.getElementById('finalScore').textContent = gameResult.finalScore || 0;
    document.getElementById('finalTime').textContent = formatTime(gameResult.timeElapsed || 0);
    document.getElementById('finalMoves').textContent = gameResult.totalMoves || 0;
    document.getElementById('finalAccuracy').textContent = `${gameResult.accuracy || 0}%`;
    
    const ratingEl = document.getElementById('performanceRating');
    if (gameResult.performance) {
        ratingEl.querySelector('.rating-emoji').textContent = gameResult.performance.level === 'Legendary' ? '👑' : '⭐';
        ratingEl.querySelector('.rating-text').textContent = gameResult.performance.level;
    }
